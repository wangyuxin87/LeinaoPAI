...
from graphs.models.mtcnn import PNet, RNet, ONet
...
def create_mtcnn_net(p_model_path=None, r_model_path=None, o_model_path=None, use_cuda=True):
    ...
    if p_model_path is not None:
        ...
        if use_cuda:
            pnet.load_state_dict(torch.load(p_model_path)['state_dict'])
            pnet.cuda()
        else:
            # forcing all GPU tensors to be in CPU while loading
            pnet.load_state_dict(torch.load(p_model_path, map_location=lambda storage, loc: storage)['state_dict'])
        pnet.eval()

    if r_model_path is not None:
        rnet = RNet(use_cuda=use_cuda)
        if use_cuda:
            rnet.load_state_dict(torch.load(r_model_path)['state_dict'])
            rnet.cuda()
        else:
            rnet.load_state_dict(torch.load(r_model_path, map_location=lambda storage, loc: storage)['state_dict'])
        rnet.eval()

    if o_model_path is not None:
        onet = ONet(use_cuda=use_cuda)
        if use_cuda:
            onet.load_state_dict(torch.load(o_model_path)['state_dict'])
            onet.cuda()
        else:
            onet.load_state_dict(torch.load(o_model_path, map_location=lambda storage, loc: storage)['state_dict'])
        onet.eval()
...
class MtcnnDetector(object):
    ...
    def detect_onet(self, im, dets):
        ...
        boxes_align = boxes_align.T
        tmp = []
        for idx in range(keep_landmark.shape[1]):
            if idx % 2 == 0:
                tmp.append(align_landmark_topx + keep_landmark[:, idx] * bw)
            else:
                tmp.append(align_landmark_topy + keep_landmark[:, idx] * bh)
        landmark = np.vstack(tmp)
