...
def gen_rnet_data(pnet_model_file, use_cuda=True, vis=False):
    ...
    for im in test_dataloader:
        ...
        if boxes_align is None:
            ...
            continue
        if vis:
            vision.vis_two(np.asarray(im), boxes, boxes_align)
    ...
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    save_file = os.path.join(save_path, "detections_pnet.pkl")
    ...
    return post_file, neg_file, part_file
...
def gen_rnet_sample_data(data_dir, anno_file, det_boxs_file, prefix_path):
    ...
    post_save_file = os.path.join(anno_store_dir, "pos_24.txt")
    neg_save_file = os.path.join(anno_store_dir, "neg_24.txt")
    part_save_file = os.path.join(anno_store_dir, "part_24.txt")
